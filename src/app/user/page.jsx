...
const columns = ['Nombre', 'Mail', 'Teléfono', 'Último Acceso'];
...
function Users() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await dataAllUsers();
        if(response == null) return setError("Error al cargar los datos del usuario");
        setUsers(response)
      } catch (error) {
  ...
  // const filteredUsersData = usersData.filter(user => user.role === 'Usuario');
  const filteredUsersData = users.filter(user => user.role === 'Usuario');
